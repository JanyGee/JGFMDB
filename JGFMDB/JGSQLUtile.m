...
+ (NSMutableDictionary*)checkTypeForValues:(NSMutableDictionary*)valuesDict
                                     class:(Class)clazz
{
    NSMutableDictionary *pdict = [[JGSQLUtile propertiesForClass:clazz] mutableCopy];
...
+ (NSString*)sqlCreateTable:(Class)clazz
    ...
    NSMutableDictionary* propertyDict = [[JGSQLUtile propertiesForClass:clazz] mutableCopy];
    ...
    if ([clazz conformsToProtocol:@protocol(JGDBDelegate)] && [clazz respondsToSelector:@selector(columnPrimaryKey)]) {
    ...
    for (NSString *property in [propertyDict allKeys]) {
        ...
        NSString *coumnType = [JGSQLUtile columnTypeWithClass:clazz dict:propertyDict name:property];
        if (coumnType) {
            if (extra) {
...
+ (NSString *)sqlConditionClazz:(Class)clz
    ...
    if ([clz conformsToProtocol:@protocol(JGDBDelegate)]) {
