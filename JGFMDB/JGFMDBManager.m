...
#import "JGFMDBManager.h"
#import "JGFMDBUtils.h"
#import "FMDatabase.h"
#import "FMDatabaseAdditions.h"

@implementation JGFMDBManager
...
- (BOOL)createTableWithTableName:(NSString *)tablename
    ...
    NSString *createSql = [JGSQLUtile sqlCreateTable:clz
                                           tableName:tablename];
    __block BOOL ret = NO;
...
- (NSUInteger)insertTableParameFromArray:(NSArray *)array
    ...
    [_db inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        for (id obj in array) {
            ...
            NSMutableDictionary *param = [JGFMDBUtils mutableDictionaryForObject:obj];
            param = [JGFMDBUtils checkTypeForValues:param class:clz];
            ...
            if (update) {
                ...
                NSInteger count = 0;
                NSString *condition = [JGFMDBUtils sqlConditionClazz:clz param:param];
                if (!condition || ![condition length]) {
                    sqlQuery = [JGFMDBUtils sqlInsertOrReplaceClazz:clz tableName:tablename param:param obj:obj];
                } else{
                    
                    condition = [NSMutableString stringWithString:[condition substringToIndex:[condition length]-4]];
                    NSString *sqlQueryCount = [JGFMDBUtils sqlQueryCountClazz:clz tableName:tablename where:condition];
                    ...
                    if (count > 0) {
                        sqlQuery = [JGFMDBUtils sqlUpdateClazz:clz tableName:tablename param:param where:condition];
                    } else{
                        sqlQuery = [JGFMDBUtils sqlInsertClazz:clz tableName:tablename param:param obj:obj];
                    }
                }
...
- (BOOL)deleteTableParameFromClazz:(Class)clz
    ...
    __block BOOL ret = NO;
    NSString *sqlUpdate = [JGFMDBUtils sqlDeleteClazz:clz tableName:tablename where:condition];
    if (!sqlUpdate) {
        ret = NO;
        sqlUpdate=@"";
    }
...
- (BOOL)updateTableParameFromObj:(id)obj
    ...
    NSMutableDictionary *param = [JGFMDBUtils mutableDictionaryForObject:obj];
    param = [JGFMDBUtils checkTypeForValues:param class:[obj class]];
    NSMutableArray *arguments = [[NSMutableArray alloc] init];
    for (NSString *name in [param allKeys]) {
        [arguments addObject:(param[name]?:@"")];
    }
    __block BOOL ret = NO;
    NSString *sqlUpdate = [JGFMDBUtils sqlUpdateClazz:[obj class]tableName:tablename param:param where:condition];
...
- (NSMutableArray *)querySql:(NSString *)sql
    ...
    [_db inDatabase:^(FMDatabase *db) {
        ...
        while ([set next]) {
            [array addObject:[JGFMDBUtils objectWithClass:clz
                                                  fromResultSet:set]];
        }
        [set close];
    }];
...
- (NSArray *)queryTableFromClazz:(Class)clz tableName:(NSString *)tablename where:(NSString *)condition{
    
    if (![self tableExsistWithTableName:tablename
                                  clazz:clz]){
        return @[];
    }
    NSString *sqlQuery = [JGFMDBUtils sqlQueryClazz:clz
                                               tableName:tablename
                                                   where:condition];
...
- (NSInteger)queryCountTableFromClazz:(Class)clz where:(NSString *)condition{
...
- (NSInteger)queryCountTableFromClazz:(Class)clz
    ...
    NSString *sqlQuery = [JGFMDBUtils sqlQueryCountClazz:clz tableName:tablename where:condition];
